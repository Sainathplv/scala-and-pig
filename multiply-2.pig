/* here are the first two lines that loads the both text files to load them as tables in hdfs/pig storage  werei,j,v orj,k,v be the digits format of a matrix in txt file  were M and N are given as input args*/
/* m = LOAD 'M-matrix-small.txt' USING PigStorage(',') AS (i,j,v:double);
n = LOAD 'N-matrix-small.txt' USING PigStorage(',') AS (j,k,v:double);
 */
m = LOAD '$M' USING PigStorage(',') AS (i,j,v:double);
n = LOAD '$N' USING PigStorage(',') AS (j,k,v:double);
...
/* output format as per given args */
STORE multiplied INTO '$O' USING PigStorage (',');
